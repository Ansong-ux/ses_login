...
// Add missing functions that are causing import errors
export async function getAvailableCourses(studentId: number) {
  return mockData.courses.map(course => ({
    ...
    prerequisites: 'None',
    max_capacity: 30,
    enrolled_count: Math.floor(Math.random() * 20) + 5,
    available_seats: 30 - (Math.floor(Math.random() * 20) + 5),
    lecturer_name: mockData.lecturers[Math.floor(Math.random() * mockData.lecturers.length)]?.first_name + ' ' + mockData.lecturers[Math.floor(Math.random() * mockData.lecturers.length)]?.last_name || 'TBA',
    lecturer_email: mockData.lecturers[Math.floor(Math.random() * mockData.lecturers.length)]?.email || 'lecturer@example.com'
  }));
}

export async function getStudentEnrollments(studentId: number) {
  return mockData.courses.slice(0, 4).map(course => ({
    enrollment_id: Math.floor(Math.random() * 1000) + 1,
    course_id: course.id,
...
export async function registerForCourse(studentId: number, courseId: number) {
  console.log(`Student ${studentId} registered for course ${courseId}`);
  return { success: true };
}

export async function dropCourse(studentId: number, courseId: number) {
  console.log(`Student ${studentId} dropped course ${courseId}`);
  return { success: true };
}
...
export async function getStudentGrades(studentId: number) {
  return [
    { course_id: 1, course_name: 'Engineering Mathematics', grade: 'A' },
    { course_id: 2, course_name: 'Applied Electricity', grade: 'B' },
    { course_id: 3, course_name: 'Engineering Graphics', grade: 'A' }
  ];
}
...
export async function getStudentDashboardData(studentId: number) {
  ...
  return {
    ...
    gpa: '3.8', // Mock value
    completed_credits: 60, // Mock value
    upcoming_assignments: mockData.assignments.length,
    recent_announcements: mockData.announcements.length,
    outstanding_fees: 500.00 // Mock value
  };
}

export async function getPaymentHistory() {
  return [
    { transaction_id: 'TXN123', date: '2024-09-15', amount: 500.00, status: 'Completed', description: 'Tuition Fees' },
    { transaction_id: 'TXN124', date: '2024-10-01', amount: 250.00, status: 'Completed', description: 'Hostel Fees' }
  ];
}
  ...
  return [
    {
      ...
      assignment_id: 1,
      grade: 85,
      feedback: 'Good work, but please double-check your calculations on question 5.',
    },
    {
      student_id: 2,
      assignment_id: 1,
...
export async function upsertGrade(gradeData: any) {
  console.log('Grade upserted:', gradeData);
  return { success: true };
} 